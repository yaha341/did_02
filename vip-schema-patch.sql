-- VIP schema for did_02 (safe to re-run).
-- Run in Supabase → SQL Editor → Run
-- After COMPLETE-SETUP.sql (shop tables), or on an existing DB.

-- Ensure touch_updated_at exists (COMPLETE-SETUP creates it)
...
-- Ensure allowed statuses include cancelled (reject flow)
DO $$
BEGIN
...
-- Personal tariff per user (legacy / hidden price group)
